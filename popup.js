...
class PopupManager {
  ...
  createPromptGroup(result) {
    ...
    if (result.shadowQueries && result.shadowQueries.length > 0) {
      ...
      result.shadowQueries.forEach(query => {
        ...
        const badgeText = query.hidden ? 'Hidden' : 'Visible';
  ...
  handleExportCSV() {
    ...
    const rows = [
      ['Generated by Alli AI Query Fan-Out Analyzer — alliai.com'],
      [],
      ['User Prompt', 'Fan-Out Query', 'Type', 'Source URL', 'Source Title']
    ];
